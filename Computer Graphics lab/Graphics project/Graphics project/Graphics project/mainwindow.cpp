    ...
    for(int i=xval;i<xval+val;i++){
        for(int j=yval;j<yval+val;j++){
            if(c==0){
                ...
                img.setPixel(j,i,qRgb(255,248,220));
            }
            else if(c==101){
...
void MainWindow::building() {
    ...
    for (int i = s; i < s + 15; i++) {
        for (int j = 5; j <= 25; j++) {
            if ((i >= s + 1 && i <= s + 2 && j >= 11 && j <= 12) ||
                (i >= s + 12 && i <= s + 13 && j >= 11 && j <= 12) ||
                (i >= s + 1 && i <= s + 2 && j >= 6 && j <= 8) ||
                (i >= s + 12 && i <= s + 13 && j >= 6 && j <= 8))
                background[i][j] = 21;  // black
            else
            {
                if(j!=25)
                background[i][j] = 31;  // yellow
                else {
                background[i][j] = 71;   //brown
                }
            }
        }
    }
    ...
    for (int i = s; i < s + 15; i++) {
        for (int j = 7; j <= 27; j++) {
            if ((i >= s + 1 && i <= s + 2 && j >= 13 && j <= 14) ||
                (i >= s + 12 && i <= s + 13 && j >= 13 && j <= 14) ||
                (i >= s + 1 && i <= s + 2 && j >= 8 && j <= 9) ||
                (i >= s + 12 && i <= s + 13 && j >= 8 && j <= 9))
                background[i][j] = 21;  // black
            else
            {
                if(j!=27)
    ...
    for (int i = s; i < s + 15; i++) {
        for (int j = 6; j <= 26; j++) {
            if ((i >= s + 1 && i <= s + 2 && j >= 12 && j <= 13) ||
                (i >= s + 12 && i <= s + 13 && j >= 12 && j <= 13) ||
                (i >= s + 1 && i <= s + 2 && j >= 7 && j <= 8) ||
                (i >= s + 12 && i <= s + 13 && j >= 7 && j <= 8))
                background[i][j] = 21;  // black
            else
            {
                if(j!=26)
...
void MainWindow::drawStars() {
    // Randomly add stars to the black sky
    for (int i = 0; i < 70; i++) {
        for (int j = 0; j < 70; j++) {
            if (background[i][j] == 0 && rand() % 200 < 5) { //star population
                background[i][j] = 100; //white stars
            }
        }
    }
}
