        ...
        while curr_frame < frames_len:
            ...
            while True:
                ...
                if isBusy==False:
                    ...
                    if rand_num<=p :#take the control of the channel
                        isBusy=True
                        can_use=True
                    else:
                        decision_val=1#wait for slot time
                else:
                    decision_val=2#wait for random time
                sem.release()
                if(can_use):
                    break
                elif decision_val==1:
                    print(f"Station {self.station_no} waiting for slot time {slot_time} s")
                    time.sleep(slot_time)
                elif decision_val==2:
                    rand_time=random.randint(0,150)/100
