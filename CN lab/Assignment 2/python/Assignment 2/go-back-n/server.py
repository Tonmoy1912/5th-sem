...
while True:
    try:
        ...
        if(waiting_for==int(seq) and ack_message=="ACK"):
            ...
            window=window[1:]
            new_frame_no=waiting_for+window_size
            if new_frame_no<frames_len:
                window.append(new_frame_no)
                


    except socket.timeout:
        print(f"Timeout: No ACK received for frame {window[0]}. Resending window: {window}.")
        # sendWindow(client_socket,window,sender)
